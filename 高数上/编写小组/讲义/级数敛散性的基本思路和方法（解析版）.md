...
>$0<r<1$ 时，$\lim\limits_{n\to\infty}r^n=0,$ 所以$\lim\limits_{n\to\infty}S_n=\dfrac{a}{1-r}.$
>$r>1$ 时，$\lim\limits_{n\to\infty}r^n=+\infty,$ 所以 $\{S_n\}$ 发散.
...
$\displaystyle(4)a^{\frac{1}{n}}\to1(n\to\infty)，由级数收敛的必要条件知，级数发散$
