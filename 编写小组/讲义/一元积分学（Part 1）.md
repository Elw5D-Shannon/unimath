...
>对一个定义在区间 $X$ 上的函数 $f(x)$，如果存在一个可导函数 $F(x)$，使得$$F'(x)=f(x),$$则称 $F(x)$ 是 $f(x)$ 的一个原函数。 $f(x)$ 的原函数全体称为其不定积分，记作$$\int f(x)\text dx=F(x)+C,$$其中 $C$ 是常数。
...
<br>
然后是连接不定积分与定积分桥梁：牛顿-莱布尼兹定理

>[!info] 牛顿-莱布尼兹定理
>设 $f(x)$ 在区间 $[a,b]$ 上可积，且其一个原函数 $F(x)$ 在区间 $[a,b]$ 上**连续**，则$$\int_a^bf(x)\text dx=F(b)-F(a).$$
